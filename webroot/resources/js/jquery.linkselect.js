...
(function($){
    ...
    $.extend({
		...
		get : function(str){
			return  (str||location.hash).replace(/^(#+)/g, ""); 
		}
    });    
	...
	$.LinkSelect = function (el, options){
		...
		$ul
			...
			.delegate("li", "click.linkselect", function (e, nofocus, doCallback){
				...
				$input.triggerHandler("update", [value, text, this]);
				$select.triggerHandler("update", [value, text, this]);
		...
		function position($el){
			...
			var pos = $.extend($el.offset(),{
				...
				, marginLeft: parseInt($.css($el[0], "marginLeft", true), 10) || 0
				, marginRight: parseInt($.css($el[0], "marginRight", true), 10) || 0
				, marginTop: parseInt($.css($el[0], "marginTop", true), 10) || 0
				, marginBottom: parseInt($.css($el[0], "marginBottom", true), 10) || 0
				, paddingLeft: parseInt($.css($el[0], "paddingLeft", true), 10) || 0
				, paddingRight: parseInt($.css($el[0], "paddingRight", true), 10) || 0
				, borderLeftWidth: parseInt($.css($el[0], "borderLeftWidth", true), 10) || 0
				, borderRightWidth: parseInt($.css($el[0], "borderRightWidth", true), 10) || 0
			});
