    ...
    var generateHtml = function (data) {
        ...
        if(data.type=="text"){
           if(data.description.length < 90 ){
        	...
        	if(data.description.length < 90 ){
          	   list.push('        <span>作品描述：</span><p style="word-break:break-all;overflow:auto;">' + data.description+'</p>');
            }else{
          	   list.push('        <span>作品描述：</span><p style="word-break:break-all;overflow:auto;">' + data.description.substr(0,90)+"......"+'</p>');
            }
        }
        ...
        if(data.type=="voice"||data.type=="video"){
	        if(data.img !=""){
	        	list.push('    <div class="audio" url="' + data.url + '" works_id="' + data.works_id + '"><img src="'+data.img+'" style="width:290px;height:161px;"><div class="playButton"></div></div>');	
	        }else{
	        	list.push('    <div class="audio" url="' + data.url + '" works_id="' + data.works_id + '"></div>');
	        }
        }else if(data.type=="image"){
        	list.push('    <div  class="image" works_id="' + data.works_id + '" style="text-align:center;"><img src="' + data.img + '" /></div>');
        }
            ...
            dataList.forEach(function (data, i) {
                ...
                (function (jqItem) {
                    ...
                    setTimeout(function () {
                        ...
                        // 设置事件
                        jqItem.find(".audio,.weixin,.description,.image").click(function () {
